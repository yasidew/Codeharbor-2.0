...
def generate_fixed_code(snippet, guideline=""):
    """
    Uses OpenAI to generate a fixed/improved version of the code snippet.
    If a company guideline is provided, the fix aligns with it.
    """
    try:
        system_prompt = (
            "You are an expert code fixer. Improve the following code by resolving issues related to security, performance, readability, maintainability, etc.\n"
            "Only return the fully rewritten code. Do not include explanations or summaries.\n"
            "Keep original formatting and variable names unless absolutely necessary."
        )
        ...
        for file_data in all_code_snippets:
            ...
            for line_num, snippet in enumerate(snippets, start=1):
                ...
                if existing_snippet:
                    ...
                    model_suggestion = existing_snippet.model_suggestion
                    ai_suggestion = existing_snippet.ai_suggestion
                    fixed_code = ""  # Skip generating again for reused snippets
                    if not ai_suggestion:
                        continue
                else:
                    print(f"🚀 Running AI analysis for snippet in {file_name}, Line {line_num}")
                ...
                suggestions.append({
                    ...
                    "severity": severity,
                    "line": line_num,
                    "fixed_code": fixed_code if fixed_code else "N/A"
                })

        # ✅ Generate the fully fixed code based on the entire code block
        full_fixed_code = generate_fixed_code(code_snippet, guideline=company_guideline_text)
        ...
        if uploaded_file.name.endswith('.csv'):
            ...
            wcc_values = np.array(
                [502, 219, 219, 554, 195, 167, 248, 646, 101, 306, 191, 399, 616, 220, 205, 246, 291, 306, 293, 298,
                 ...
                 397, 645, 237, 196, 241, 551, 224, 188, 212, 257, 288, 257, 230, 210, 600, 386, 555, 145, 665, 576, 542, 605, 625, 587, 615, 672, 435, 718, 345, 773, 463, 589, 566, 337, 231]).reshape(-1, 1)
