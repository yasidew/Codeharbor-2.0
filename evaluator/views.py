...
#         suggestion = simple_inference("qwen2.5-7b-instruct-1m", suggestion_input)
#         if suggestion:
#             suggestions.append({
#                 "guideline": guideline,
...
def fetch_html_from_url(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            url = data.get("url")
...
def load_from_url_modal(request):
    return render(request, 'evaluator/url_fetch_modal.html')